...
sudo tar -czvf sl-func-files.tar.gz $source_sl_path && ssh $destination_ssh "tar -xzv" < sl-func-files.tar.gz
...
echo "Copy serverless function files from source to destination (seconds) -- tar|scp|untar = " >> $latency_file_name
